...
- **Shader Defense Dance**: fullscreen p5 WebGL shader with radial defensive patterning inspired by the supplied Creative Coding Challenge style.
- **Shader Organism Field**: cellular membranes, veins, and note-reactive biological motion.
- **Shader Cathedral Glass**: stained-glass arches, columns, and MIDI-controlled pattern density.
...
The main canvas now fills the available viewport and includes a **Fullscreen Canvas** control. A right-side live parameter sidebar exposes shader distortion, symmetry, repetition, morphing, bloom, line weight, feedback, particle controls, scene controls, virtual MIDI knobs, and editable color palettes.
...
Continuous controller values are normalized to `0–1`, pitch bend is normalized to `-1–1`, and keys/pads update trigger state. Visual modules consume `VisualState`, not raw MIDI messages. Shader parameters live in `visualState.shader` so MIDI mappings and UI sliders can control the same real-time values.
